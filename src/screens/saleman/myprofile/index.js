...
class User extends React.PureComponent {
  ...
  _editNickName(){
    this.props.navigation.navigate("editNickName",{route:"SaleProfile"});
  }
  _editEmpName(){
    this.props.navigation.navigate("editName",{route:"SaleProfile"});
  }

  _editPhone(){
    this.props.navigation.navigate("editPhone",{route:"SaleProfile"});
  }
  _editStore(){
    this.props.navigation.navigate("Admin_Store",{data:this.state.data});
  }
  ...
  render() {
    ...
    return (
      ...
      <Header_Sale navigation={this.props.navigation} radio1={variables.headerTextPrimary}/>
      {(uploading)?
          <Card style={styles.logo} >
            ...
            <TouchableOpacity style={{alignSelf:'center'}} onPress={()=>{this.getPermissionAsync()}} transparent>
              <View>
                ...
                />
                  <Icon name='camera' style={{position: 'absolute',right: 0,bottom: 0, color:variables.headerTextPrimary}}/>
              </View>
            </TouchableOpacity>
              : 
              <TouchableOpacity style={{alignSelf:'center'}} onPress={()=>{this.getPermissionAsync()}} transparent>
          ...
          <Card style={{ flex:1 }}>
          <Content style={{marginTop:10, marginLeft:8}}>
          <TouchableOpacity
          ...
          <Button 
                    onPress = {()=>{ this.props.navigation.navigate("Changepass",{emp_id:this.props.emp_id,route:"SaleProfile"})}}
                    transparent block>
              <Text style={{ color: variables.textPrimary9 }}>เปลี่ยนรหัสผ่าน</Text>
           </Button>
          </Content>
          </Card>
      </Container>
