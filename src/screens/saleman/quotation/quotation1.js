...
class Quotation1 extends React.PureComponent {
  ...
  __getMyClient = async ()=>{
    this.setState({
      ...
      showdata:null,
      data:null});
      AUTHEN_POST('/quatation/MyQuotationClient',{SalesCode:this.props.emp_id})
      .then(async (response)=>{
  ...
  __getQuotationByCV= async (item)=>{
    this.setState({loading:true});
      ...
      .then(async (response)=>{
        this.setState({loading:false});
        if(response.data.length > 0){
          this.props.navigation.navigate("SelectQuot", {Salescode:this.props.emp_id ,CV:item,data:response.data});
        }else{
          this.props.navigation.navigate("Quotation2", {Salescode:this.props.emp_id ,data:item});
        }
      })
      .catch((error)=>{
        this.setState({loading:false});
  ...
  _onGoback(){
    this.props.navigation.navigate("Sale_Menu")
  }
